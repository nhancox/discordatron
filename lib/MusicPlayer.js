...
class MusicPlayer {
  ...
  async next(startLoop) {
    ...
    try {
      ...
      const musicStream = await ytdl(
        this.nowPlaying.musicInfo.videoDetails.video_url,
        {
          filter: "audioonly",
      ...
      dispatcher.on("start", async () => {
        if (!this.loopMode || startLoop) {
          await this.nowPlaying.message.channel.send(
            `Now Playing: ${this.nowPlaying.musicInfo.videoDetails.title} (requested by ${this.nowPlaying.message.author})`
          );
        }
      });
  ...
  async queue(message) {
    if (this.loopMode) {
      await message
        .reply(
          `Currently looping: ${this.nowPlaying.musicInfo.videoDetails.title}`
        )
        .catch(console.error);
      return;
    }
    ...
    if (!this.nowPlaying && !this.musicQueue.length) {
      ...
      if (this.nowPlaying) {
        queueList += `\nCurrent song: ${this.nowPlaying.musicInfo.videoDetails.title}`;
      } else {
        queueList += "\nCurrent song: [NONE]";
      }
      ...
      if (this.musicQueue.length) {
        ...
        this.musicQueue.forEach((entry, index) => {
          queueList += `\n  ${index + 1}. ${
            entry.musicInfo.videoDetails.title
          }`;
        });
      } else {
        queueList += "\nQueue: [EMPTY]";
  ...
  async remove(message) {
    ...
    await message
      .reply(
        `Removed ${removedEntry.musicInfo.videoDetails.title} from position ${requestedRemovePosition} (originally requested by ${removedEntry.message.author}).`
      )
      .catch(console.error);
  }
